...
def fetch_real_air_telemetry(lat: float, lng: float) -> Dict[str, Any]:
    ...
    and 24-hour hourly curve for exact geographic coordinates.
    """
    # 1. Reverse geocoding to get official place name
    location_name = "Targeted Zone"
    try:
        geo_url = f"https://nominatim.openstreetmap.org/reverse?format=json&lat={lat}&lon={lng}&zoom=14"
        headers = {"User-Agent": "PraanVayu-Analytics/2.0"}
        geo_res = requests.get(geo_url, headers=headers, timeout=4)
        if geo_res.status_code == 200:
            addr = geo_res.json().get("address", {})
            suburb = addr.get("suburb") or addr.get("neighbourhood") or addr.get("city_district") or addr.get("village") or addr.get("county") or ""
        ...
        print(f"[Geo] Reverse geocoding fallback: {e}")

    # 2. Live Air Quality (US AQI, PM2.5, PM10, CO, NO2)
    aqi = 165
    pm25 = 85.0
    pm10 = 140.0
    co = 450.0
    ...
    try:
        ...
        if aq_res.status_code == 200:
            ...
            # Format hourly diurnal curve (6 key points for chart)
            h_data = data.get("hourly", {})
            times = h_data.get("time", [])
            aqi_list = h_data.get("us_aqi", [])
            pm25_list = h_data.get("pm2_5", [])
            
            # Step every 4 hours for clean chart representation
    ...
    # Fallback hourly if API was slow
    if not hourly_curve:
        hourly_curve = [
            {"time": "06:00", "aqi": max(50, aqi - 30), "pm25": max(25.0, pm25 - 20)},
            {"time": "10:00", "aqi": aqi + 15, "pm25": pm25 + 10},
            {"time": "14:00", "aqi": max(50, aqi - 10), "pm25": pm25},
            {"time": "18:00", "aqi": aqi + 25, "pm25": pm25 + 18},
            {"time": "22:00", "aqi": aqi + 20, "pm25": pm25 + 12}
        ]

    # 3. Live Weather (Temperature, Relative Humidity, Wind Speed)
    temperature = 32.0
    humidity = 42
    wind_speed = 12.0
